...
typedef signed long long int	Word64;
...
void PolyphaseMono(short *pcm, int *vbuf, const int *coefBase, chanend pcmChan)
{	
#if 0
	int i;
	const int *coef;
	...
	OutputToPCMBuf(ClipToShort((int)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS), pcmIdx + 16, pcmChan);
	...
	for (i = 15; i > 0; i--) {
		...
		MC2M(7)
		// printDebug(sum1L, debugChan);
		// printDebug(sum2L, debugChan);

		vb1 += 64;
		OutputToPCMBuf(ClipToShort((int)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS), pcmIdx, pcmChan);
		// *(pcm)       = ClipToShort((int)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS);
		OutputToPCMBuf(ClipToShort((int)SAR64(sum2L, (32-CSHIFT)), DEF_NFRACBITS), pcmIdx + 2*i, pcmChan);
