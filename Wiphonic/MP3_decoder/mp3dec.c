 ...
 **************************************************************************************/
int MP3Decode(HMP3Decoder hMP3Decoder, unsigned char **inbuf, int *bytesLeft, short *outbuf, int useSize, chanend pcmChan)
{
	...
	*bytesLeft -= (fhBytes + siBytes);
	...
	for (gr = 0; gr < mp3DecInfo->nGrans; gr++)
	{
		...
		if (Subband(mp3DecInfo, outbuf + gr*mp3DecInfo->nGranSamps*mp3DecInfo->nChans, pcmChan) < 0)
		{
			MP3ClearBadFrame(mp3DecInfo, pcmChan);
