 ...
 **************************************************************************************/
int Subband(MP3DecInfo *mp3DecInfo, short *pcmBuf, chanend pcmChan)
{
	int b;
	HuffmanInfo *hi;
	IMDCTInfo *mi;
	...
	if (mp3DecInfo->nChans == 2)
	{
		...
		for (b = 0; b < BLOCK_SIZE; b++)
		{
			...
			PolyphaseStereo(pcmBuf, sbi->vbuf + sbi->vindex + VBUF_LENGTH * (b & 0x01), polyCoef, pcmChan);
	...
	} else
	{
		...
		{
			FDCT32(mi->outBuf[0][b], sbi->vbuf + 0*32, sbi->vindex, (b & 0x01), mi->gb[0]);
			PolyphaseMono(pcmBuf, sbi->vbuf + sbi->vindex + VBUF_LENGTH * (b & 0x01), polyCoef, pcmChan);
			sbi->vindex = (sbi->vindex - (b & 0x01)) & 7;
			// pcmBuf += NBANDS;
		}
	}
